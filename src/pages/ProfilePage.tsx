...
export default function ProfilePage() {
    ...
    return (
        <div className="cv-animate-fadeIn" style={{ maxWidth: 640, margin: '0 auto', paddingBottom: '2.5rem' }}>
            <div className="cv-card" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '1.5rem', padding: '1.75rem 2rem', background: 'linear-gradient(135deg, var(--cv-primary-light) 0%, var(--cv-surface) 100%)' }}>
                <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg, var(--cv-primary), var(--cv-primary-dark))', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.6rem', fontWeight: 800, flexShrink: 0 }}>
                    {initials}
                </div>
                <div style={{ flex: 1 }}>
            ...
            <div className="cv-grid cv-grid-3" style={{ marginBottom: '1.5rem' }}>
                {[
                    { icon: '📋', label: t.issuesReported, val: myIssues.length },
                    { icon: '✅', label: t.resolved, val: myResolved },
                    { icon: '👍', label: t.totalUpvotes, val: myUpvotes },
                ].map((s) => (
                    <div key={s.label} className="cv-card" style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '1.5rem', marginBottom: '.25rem' }}>{s.icon}</div>
                        <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--cv-primary)' }}>{s.val}</div>
                ...
                <div className="cv-card">
                    ...
                    <div className="cv-flex-col cv-gap-sm">
                        {myIssues.slice(0, 5).map((issue) => (
                            <Link key={issue.id} to={`/issues/${issue.id}`} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '.6rem .75rem', borderRadius: 'var(--cv-radius)', background: 'var(--cv-surface-alt)', textDecoration: 'none', color: 'var(--cv-text)', fontSize: '.875rem', gap: '.75rem' }}>
                                <span style={{ flex: 1, fontWeight: 500 }}>{issue.title.length > 55 ? issue.title.slice(0, 55) + '…' : issue.title}</span>
                                <span style={{ padding: '.2rem .5rem', borderRadius: 99, fontSize: '.72rem', fontWeight: 600, textTransform: 'capitalize', background: issue.status === 'resolved' || issue.status === 'closed' ? '#10B98118' : '#F59E0B18', color: issue.status === 'resolved' || issue.status === 'closed' ? '#10B981' : '#F59E0B', whiteSpace: 'nowrap' }}>
