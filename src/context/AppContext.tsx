...
export function AppProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        async function loadIssues() {
            try {
                ...
                if (error) {
                    console.error('[CityVaani] Supabase fetch error:', error.message);
                    setIssues([...MOCK_ISSUES]);
                    setIssuesLoading(false);
                    return;
                }
                ...
                if (!data || data.length === 0) {
                    ...
                    if (currentUserRef.current?.id && currentUserRef.current?.id !== 'guest') {
                        console.info('[CityVaani] Seeding Supabase with mock issues…');
                        ...
                        if (seedErr) {
                            console.error('[CityVaani] Seed error:', seedErr.message);
                        }
                    }
                    setIssues([...MOCK_ISSUES]);
                } else {
                ...
                console.error('[CityVaani] Critical load issues error:', err);
                alert('[CityVaani Debug] loadIssues Crashed: ' + err.message);
                setIssues([...MOCK_ISSUES]);
                setIssuesLoading(false);
            }
        }
    ...
    const login = useCallback(async (email: string, role?: UserRole) => {
        ...
        if (HAS_SUPABASE) {
            ...
            if (error) console.error('[CityVaani] User upsert error:', error.message);
        }
    }, []);
    ...
    const addIssue = useCallback(
        async (draft: Omit<Issue, 'id' | 'createdAt' | 'updatedAt' | 'upvotes' | 'upvotedBy'>) => {
            ...
            if (HAS_SUPABASE) {
                ...
                if (error) {
                    console.error('[CityVaani] addIssue error:', error.message);
    ...
    const updateIssueStatus = useCallback(async (issueId: string, status: IssueStatus) => {
        ...
        if (HAS_SUPABASE) {
            const { error } = await supabase
                ...
                .eq('id', issueId);
            if (error) {
                console.error('[CityVaani] updateStatus error:', error.message);
    ...
    const upvoteIssue = useCallback(async (issueId: string) => {
        ...
        if (HAS_SUPABASE && !alreadyVoted) {
            const { error } = await supabase
                ...
                .eq('id', issueId);
            if (error) {
                console.error('[CityVaani] upvote error:', error.message);
